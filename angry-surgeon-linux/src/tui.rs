...
pub struct Scene {
    pub bank_a: [[Pad; PAD_COUNT]; PAD_COUNT],
    pub bank_b: [[Pad; PAD_COUNT]; PAD_COUNT],
}

impl Scene {
    pub fn empty() -> Self {
        Self {
            bank_a: [[Pad::new(); PAD_COUNT]; PAD_COUNT],
            bank_b: [[Pad::new(); PAD_COUNT]; PAD_COUNT],
        }
    }

    pub fn from_audio(scene: &angry_surgeon_core::Scene<BANK_COUNT, PAD_COUNT, MAX_PHRASE_LEN>) -> Self {
        let bank_a = core::array::from_fn(|i| {
            core::array::from_fn(|j| {
                let pad = &scene.banks[0].kits[i].pads[j];
                Pad {
        ...
        let bank_b = core::array::from_fn(|i| {
            core::array::from_fn(|j| {
                let pad = &scene.banks[1].kits[i].pads[j];
                Pad {
                    onsets: [pad.onsets[1].is_some(), pad.onsets[1].is_some()],
                    phrase: pad.phrase.is_some(),
                }
            })
        });
    ...
    fn cmd(&mut self, cmd: Cmd) {
        match cmd {
            ...
            Cmd::Bank(bank, cmd) => {
                ...
                match bank {
                    Bank::A => self.bank_a.cmd(&mut self.scene.bank_a, cmd),
                    Bank::B => self.bank_b.cmd(&mut self.scene.bank_b, cmd),
                }
            }
        }
    }
    ...
    fn render_load_scene(&self, paths: &[String; FILE_COUNT], area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(Text::raw(String::from_iter(core::array::from_fn::<_, PAD_COUNT, _>(|i| {
            if self.bank_a.downs.contains(&(i as u8)) {
                'o'
            } else if self.scene.bank_a[i].iter().any(|v| v.onsets[0] || v.onsets[1] || v.phrase) {
                'k'
            } else {
                '.'
            }
        ...
        Paragraph::new(Text::raw(String::from_iter(core::array::from_fn::<_, PAD_COUNT, _>(|i| {
            if self.bank_b.downs.contains(&(i as u8)) {
                'o'
            } else if self.scene.bank_b[i].iter().any(|v| v.onsets[0] || v.onsets[1] || v.phrase) {
                'k'
            } else {
                '.'
            }
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        match &self.state {
            GlobalState::Yield => {
                ...
                self.bank_a.render(&self.scene.bank_a, Flex::End, a_area, buf);
                self.bank_b.render(&self.scene.bank_b, Flex::Start, b_area, buf);
            }
